...
def get_complete_industry_data():
    """获取完整的行业分类数据"""
    print("开始获取完整行业分类数据...")
    
    # 完整的万得行业分类数据（基于Wind行业分类标准）
    industries = [
        # 一级行业 (19个)
...
def save_complete_data(industries, stocks):
    """保存完整数据到数据库"""
    try:
...
def main():
    ...
    # 获取完整行业数据
    industries = get_complete_industry_data()
    print(f"获取到 {len(industries)} 个行业分类")
    
    # 获取完整股票映射数据
    stocks = get_comprehensive_stock_data()
    print(f"获取到 {len(stocks)} 只股票映射")
    
    # 保存到数据库
    if save_complete_data(industries, stocks):
        print("\n数据采集和保存成功！")
        print("现在可以启动管理台查看完整数据了")
