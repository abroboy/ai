  ...
  fetch('/api/tencent-jian-index')
    ...
    .then(data => {
      if (data.success && data.data) {
        const jianIndexData = data.data;
        
        // 补充缺失的数据字段
        jianIndexData.components = [
        ...
        const moduleHTML = `
          <div class="mb-4">
            <h4 class="fw-bold text-primary mb-3">腾讯济安指数 <span class="badge bg-danger">实时</span></h4>
        ...
        container.innerHTML = `
          <div class="alert alert-danger">
            <i class="bi bi-exclamation-triangle-fill"></i> 无法加载腾讯济安指数数据: ${data.message || '未知错误'}
          </div>
        `;
      }
    })
